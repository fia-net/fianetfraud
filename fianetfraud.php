...
class fianetfraud extends Module
{
    ...
    $tab_controller_main = new Tab();
        ...
        return (parent::install() && $this->registerHook('newOrder') && $this->registerHook('paymentConfirm') && $this->registerHook('adminOrder') && $this->registerHook('backOfficeHeader') && $this->registerHook('backBeforePayment') && $this->registerHook('postUpdateOrderStatus'));
    }
    ...
    private function loadPaymentMethods()
    {
        ...
        Tools::getValue('certissim_' . $payment['id_module'] . '_payment_enabled') : Configuration::get('CERTISSIM_' . $payment['id_module'] . '_PAYMENT_ENABLED');
            ...
            Tools::getValue($level . '_' . $payment['id_module']) : Configuration::get($levelup . '_' . $payment['id_module']);
    ...
    private function formIsValid()
    {
        ...
        if (!in_array(Tools::getValue('certissim_copilot_status'), array('0', '1'))) {
            $this->_errors[] = $this->l('You must give a correct FIA-NET Copilot status value');
        }
        if (!in_array(Tools::getValue('certissim_franfinance_status'), array('0', '1'))) {
        ...
        if (Tools::getValue('certissim_franfinance_status')) {
            ...
            } elseif (!preg_match('#^[0-9]+$#', Tools::getValue('certissim_franfinance_siteid'))) {
    ...
    if ($this->formIsValid()) {
        ...
        Configuration::updateValue('CERTISSIM_AUTHKEY', Tools::getValue('certissim_authkey'));
        Configuration::updateValue('CERTISSIM_COPILOT_STATUS', ((int) Tools::getValue('certissim_copilot_status') == 1 ? '1' : '0'));
        Configuration::updateValue('CERTISSIM_FRANFINANCE_SITEID', Tools::getValue('certissim_franfinance_siteid'));
        Configuration::updateValue('CERTISSIM_FRANFINANCE_STATUS', ((int) Tools::getValue('certissim_franfinance_status') == 1 ? '1' : '0'));
        ...
        foreach (array_keys($payment_modules) as $id) {
            ...
            foreach ($levels as $level) {
                $levelup = str_replace('/', '_', Tools::strtoupper($level));
                Configuration::updateValue($levelup . '_' . $id, ((int) Tools::getValue($level . '_' . $id) == 1 ? '1' : '0'));
            }
        }
    ...
    $payment_modules = $this->loadPaymentMethods();
        ...
        $certissim_authkey = Tools::isSubmit('certissim_authkey') ? Tools::getValue('certissim_authkey') : Configuration::get('CERTISSIM_AUTHKEY');
        ...
        $certissim_copilot_status = Tools::isSubmit('certissim_copilot_status') ? Tools::getValue('certissim_copilot_status') : Configuration::get('CERTISSIM_COPILOT_STATUS');
        $certissim_franfinance_status = Tools::isSubmit('certissim_franfinance_status') ? Tools::getValue('certissim_franfinance_status') : Configuration::get('CERTISSIM_FRANFINANCE_STATUS');
        $certissim_franfinance_siteid = Tools::isSubmit('certissim_franfinance_siteid') ? Tools::getValue('certissim_franfinance_siteid') : Configuration::get('CERTISSIM_FRANFINANCE_SITEID');
    ...
    if ($found) {
        ...
        CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, "id_order : " . $params['order']->id . ", Module détecté :" . $payment_name);
    ...
    switch ($order_label) {
        ...
        if ($paid) {
            ...
            if (!is_null($order_array['error']) && $order_array['error'] != '') {
                $this->smarty->assign('txt', $this->l('An error has been encountered when the order has been sent to Certissim: ') . '\'' . $order_array['error'] . '\'
        ...
        $this->smarty->assign('logo_path', __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/certissim_mini.png');
    ...
    public function buildAndSend($id_order)
    {
        ...
        if (Configuration::get('CERTISSIM_FRANFINANCE_STATUS')) {
            if ($this->getFranfinanceSolution($id_order)) {
    ...
    $xml_element_invoice_customer = new CertissimUtilisateur(
        'facturation', $gender, $address_invoice->lastname, $address_invoice->firstname, $address_invoice->company, $address_invoice->phone, $address_invoice->phone_mobile, null, $customer->email
    );
    ...
    $xml_element_invoice_customer_stats = new CertissimSiteconso(
        $customer_stats['total_orders'], $customer_stats['nb_orders'], $all_orders[count($all_orders) - 1]['date_add'], (count($all_orders) > 1 ? $all_orders[1]['date_add'] : null)
    );
    ...
    $xml_element_invoice_address = new CertissimAdresse(
        'facturation', $address_invoice->address1, $address_invoice->address2, $address_invoice->postcode, $address_invoice->city, $country->name[$id_lang]
    );
    ...
    if (!$from_id_cart) {
        ...
        if (_PS_VERSION_ >= '1.5' && Shop::isFeatureActive()) {
            if (Configuration::get('CERTISSIM_FRANFINANCE_STATUS') && in_array($payment_type, Fianetfraud::$specific_payments)) {
                $siteid = Configuration::get('CERTISSIM_FRANFINANCE_SITEID', null, null, $order->id_shop);
            } else {
    ...
    $xml_element_order_details = new CertissimInfocommande(
        $siteid, $ref, (string) $total_paid, $ip_customer, $date_order, $currency->iso_code
    );
    ...
    $alldownloadables = true;
        ...
        if (!empty($product['product_reference'])) {
            ...
            $product_ref = str_replace(array('&', "'", 'Ø', '|'), array('', '', '', ''), $product_ref);
            $product_name = str_replace(array('&', "'", 'Ø', '|'), array('', '', '', ''), $product_name);
        ...
        $xml_element_product->addAttribute('ref', CertissimTools::normalizeString($product_ref));
            $xml_element_product->addAttribute('nb', $product_quantity);
            $xml_element_product->addAttribute('prixunit', $product_price);
    ...
    elseif (_PS_VERSION_ >= '1.5' && Shop::isFeatureActive()) {
        ...
        if (in_array(Configuration::get('CERTISSIM_' . (string) $carrier->id . '_CARRIER_TYPE', null, null, $order->id_shop), array_keys($this->carrier_types))) {
    ...
    //if selected carrier fianet type not defined, uses the default one
        ...
        $xml_element_delivery_customer = new CertissimUtilisateur(
            'livraison', $customer->id_gender == 2 ? $this->l('Miss') : $this->l('Mister'), $address_delivery->lastname, $address_delivery->firstname, $address_delivery->company, $address_delivery->phone, $address_delivery->phone_mobile, null, null);
        ...
        $xml_element_delivery_address = new CertissimAdresse(
            'livraison', $address_delivery->address1, $address_delivery->address2, $address_delivery->postcode, $address_delivery->city, $country->name[$id_lang], null
        );
        $xml_pointrelais = null;
        break;
            ...
            if ($delivery_mode == 'DOM' || $delivery_mode == 'RDV') {
                ...
                'livraison', $customer->id_gender == 2 ? $this->l('Miss') : $this->l('Mister'), $address_delivery->lastname, $address_delivery->firstname, $address_delivery->company, $address_delivery->phone, $address_delivery->phone_mobile, null, null);
                    ...
                    if ($address_invoice->firstname != $firstname) {
                        $xml_element_delivery_customer = new CertissimUtilisateur(
                    'livraison', $customer->id_gender == 2 ? $this->l('Miss') : $this->l('Mister'), $name, $firstname, null, null, $mobile_phone, null, null);
                    }
                }

                $xml_element_delivery_address = new CertissimAdresse(
                'livraison', $address3, $address4, $zipcode, $city, $country, null
            );
        ...
        if ($socolissimoinfo != false) {
            ...
            if ($delivery_mode == 'DOM' || $delivery_mode == 'RDV') {
                ...
                $xml_element_delivery_address = new CertissimAdresse(
                'livraison', $address1, $address2, $zipcode, $city, $country, null
            );
        ...
        if (Module::isInstalled('chronopost') || $chronopost_is_enabled) {
            $xml_element_delivery_customer = new CertissimUtilisateur(
            'livraison', $customer->id_gender == 2 ? $this->l('Miss') : $this->l('Mister'), $address_delivery->lastname, $address_delivery->firstname, null, $address_delivery->phone, $address_delivery->phone_mobile, null, null);
        ...
        if ($real_carrier_type == 1) {
            ...
            $delivery_address = $this->getDefautAddress(Configuration::get('PS_SHOP_ADDR1'), Configuration::get('PS_SHOP_ADDR2'), Configuration::get('PS_SHOP_CODE'), Configuration::get('PS_SHOP_CITY'), Configuration::get('PS_SHOP_COUNTRY'), Configuration::get('PS_SHOP_NAME'));
            $xml_element_delivery_customer = new CertissimUtilisateur(
            'livraison', $customer->id_gender == 2 ? $this->l('Miss') : $this->l('Mister'), $address_delivery->lastname, $address_delivery->firstname, $address_delivery->company, $address_delivery->phone, $address_delivery->phone_mobile, null, null);
        } else {
            //get delivery information
            $delivery_address = $this->getDefautAddress($address_delivery->address1, $address_delivery->address2, $address_delivery->postcode, $address_delivery->city, $country->name[$id_lang], null);
        }
    ...
    $xml_element_carrier = new CertissimTransport(
        $real_carrier_type, $alldownloadables ? 'Téléchargement' : Tools::htmlentitiesUTF8($carrier->name), $alldownloadables ? '1' : $real_carrier_speed, $xml_pointrelais
    );
        ...
        if (Configuration::get('CERTISSIM_COPILOT_STATUS')) {
            ...
            if ($this->getAuthID($order->id_cart)) {
                $xml_element_payment->addChild('<authRequestID>' . $this->getAuthID($order->id_cart) . '</authRequestID>');
            }
        }
        }
    ...
    $payment_type = $order->module;
        if (Configuration::get('CERTISSIM_FRANFINANCE_STATUS') && in_array($payment_type, Fianetfraud::$specific_payments)) {
            $res = $sac->sendStacking($stack, Configuration::get('CERTISSIM_FRANFINANCE_SITEID'));
        } else {
    ...
    public function hookBackOfficeHeader()
    {
        ...
        if ($orders_id) {
            foreach ($orders_id as $value) {
                CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'Commande Franfinance envoyée : id_order = ' . $value['id_order']);
                $this->buildAndSend($value['id_order']);
            }
        }

        return '<link rel="stylesheet" type="text/css" href="' . __PS_BASE_URI__ . 'modules/' . $this->name . '/views/css/toolbarAdmin.css" />';
    }
    ...
    if (Configuration::get('CERTISSIM_COPILOT_STATUS')) {
        ...
        if (!isset($this->context->cookie->session_xml) || ($xml_ligth != $xml_session_ligth) || !$this->getAuthID($id_cart)) {
        ...
        CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'id_cart : ' . $id_cart . ', authID : ' . $authid);
        CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'id_cart : ' . $id_cart . ', authLevel : ' . $authlevel);

        self::updateCertissimOrder($id_cart, array(
        'authlevel' => $authlevel,
        'authid' => $authid), true);
    }
    }
        ...
        if ($validcopilot != false) {
            if (CertissimTools::getHttpResponse($validcopilot['header']) == '200') {
                if (CertissimTools::isXMLstring($validcopilot['response'])) {
                    ...
                    CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'id_cart ' . $id_cart . ' : reponse http : ' . $validcopilot['header'] . ', reponse copilot : ' . $validcopilot['response']);
                    return array('authlevel' => $response->getAuthLevel(), 'authid' => $response->getAuthId());
                } else {
                    CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'id_cart ' . $id_cart . ' : chaine retour pas au format xml');
                    return false;
                }
            } else {
                CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'id_cart ' . $id_cart . ' : erreur http ' . CertissimTools::getHttpResponse($validcopilot['header']) . ', libell� erreur : ' . $validcopilot['response']);
                return false;
            }
        } else {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'id_cart ' . $id_cart . ' : erreur de connexion fsockopen');
            return false;
        }
    }
    ...
    {
        $sql = 'SELECT `authlevel` FROM `' . _DB_PREFIX_ . self::CERTISSIM_ORDER_TABLE_NAME . "` WHERE `id_cart` = '" . (int) $id_cart . "'";
        $query_result = Db::getInstance()->getRow($sql);
        if ($query_result['authlevel'] != null || $query_result['authlevel'] != '') {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'authlevel found for id_cart :' . $id_cart);
            return $query_result['authlevel'];
        } else {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'authlevel not found for id_cart :' . $id_cart);
            return false;
        }
    }
    ...
    {
        $sql = 'SELECT `authid` FROM `' . _DB_PREFIX_ . self::CERTISSIM_ORDER_TABLE_NAME . "` WHERE `id_cart` = '" . (int) $id_cart . "'";
        $query_result = Db::getInstance()->getRow($sql);
        if ($query_result['authid'] != null || $query_result['authid'] != '') {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'authid found for id_cart :' . $id_cart);
            return $query_result['authid'];
        } else {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'authid not found for id_cart :' . $id_cart);
            return false;
        }
    }
    ...
    {
        $sql = 'SELECT `solution` FROM `' . _DB_PREFIX_ . self::FRANFINANCE_ORDER . "` WHERE `id_order` = '" . (int) $id_order . "'";
        $query_result = Db::getInstance()->getValue($sql);

        if ($query_result) {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'Solution Franfinance trouvée pour id_order = ' . $id_order . ', solution : ' . $query_result);
            ...
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'Solution Franfinance non trouvée pour id_order = ' . $id_order);
            return false;
        }
    }
    ...
    $sql = 'SELECT `id_order` FROM `' . _DB_PREFIX_ . self::CERTISSIM_ORDER_TABLE_NAME . "` WHERE `id_certissim_state` = 2 AND `paiement` LIKE 'franfinance'";
        $query_result = Db::getInstance()->executeS($sql);

        if ($query_result) {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'Transactions Franfinance à envoyer');
            return $query_result;
        } else {
            CertissimLogger::insertLog(__METHOD__ . ' : ' . __LINE__, 'Aucune transaction Franfinance à envoyer');
            return false;
        }
    }
}
