...
class Fianetfraud extends Module
{
    ...
    private $certissim_levels = array(
        '1' => 'high/required',
        '2' => 'high/advised',
        '3' => 'medium/required',
        '4' => 'medium/advised',
    ...
    /*     * ************* MODIF FRANFINANCE *************** */
    public static $specific_payments = array(
        'franfinance',
    );
    ...
    const FRANFINANCE_ORDER = 'franfinance_orders_solutions';
    ...
    public function install()
    {
        ...
        foreach ($queries as $query) {
            if (!Db::getInstance()->Execute(trim($query))) {
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'Install impossible, génération base échouée : ' . Db::getInstance()->getMsgError()
                );
                return false;
            }
        }
        ...
        foreach ($this->certissim_states as $id => $label) {
            ...
            if (!$insert) {
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'Insertion state $id.$label échouée : ' . Db::getInstance()->getMsgError()
                );
            }
        }
    ...
    /**
     * returns the carriers list as an array indexed by carrier_id 
     * and containing the carrier name and the corresponding fia-net carrier type
     * 
     * @return array
     */
    private function loadCarriers()
    {
        ...
        foreach ($carriers as $carrier) {
            $certissim_type = Tools::isSubmit('certissim_' . $carrier['id_carrier'] . '_carrier_type') ?
                Tools::getValue('certissim_' . $carrier['id_carrier'] . '_carrier_type')
                : Configuration::get('CERTISSIM_' . $carrier['id_carrier'] . '_CARRIER_TYPE');
            $certissim_speed = Tools::isSubmit('certissim_' . $carrier['id_carrier'] . '_carrier_speed') ?
                Tools::getValue('certissim_' . $carrier['id_carrier'] . '_carrier_speed')
                : Configuration::get('CERTISSIM_' . $carrier['id_carrier'] . '_CARRIER_SPEED');
    ...
    /**
     * returns the payment modules list as an array indexed by module_id 
     * and containing the module name and the corresponding fia-net payment type
     * 
     * @return array
     */
    private function loadPaymentMethods()
    {
        ...
        $levels = $this->certissim_levels;
        $copilot_status = array();
        ...
        foreach ($payments as $payment) {
            ...
            $certissim_type = Tools::isSubmit('certissim_' . $payment['id_module'] . '_payment_type') ?
                Tools::getValue('certissim_' . $payment['id_module'] . '_payment_type') :
                Configuration::get('CERTISSIM_' . $payment['id_module'] . '_PAYMENT_TYPE');
            $certissim_enabled = Tools::isSubmit('certissim_' . $payment['id_module'] . '_payment_enabled') ?
                Tools::getValue('certissim_' . $payment['id_module'] . '_payment_enabled') :
                Configuration::get('CERTISSIM_' . $payment['id_module'] . '_PAYMENT_ENABLED');

            foreach ($levels as $level) {
                $levelup = str_replace('/', '_', Tools::strtoupper($level));
                $level_status = Tools::isSubmit($level . '_' . $payment['id_module']) ?
                    Tools::getValue($level . '_' . $payment['id_module']) :
                    Configuration::get($levelup . '_' . $payment['id_module']);
                $copilot_status['' . $level . ''] = $level_status;
            }
    ...
    private function formIsValid()
    {
        ...
        if (!in_array(
            Tools::getValue('certissim_default_product_type'),
            array_keys($this->product_types)
        )) {
            $this->_errors[] = $this->l('You must configure a valid default product type');
        }
        if (!in_array(
            Tools::getValue('certissim_default_carrier_type'),
            array_keys($this->carrier_types)
        )) {
            $this->_errors[] = $this->l('You must configure a valid default carrier type');
        }
        if (!in_array(
            Tools::getValue('certissim_default_carrier_speed'),
            array_keys($this->carrier_speeds)
        )) {
            $this->_errors[] = $this->l('You must configure a valid default carrier speed');
        }
        if (!in_array(
            Tools::getValue('certissim_copilot_status'),
            array('0', '1')
        )) {
            $this->_errors[] = $this->l('You must give a correct FIA-NET Copilot status value');
        }
        if (!in_array(
            Tools::getValue('certissim_franfinance_status'),
            array('0', '1')
        )) {
            $this->_errors[] = $this->l('You must give a correct FIA-NET franfinance status value');
        }

        if (Tools::getValue('certissim_copilot_status')) {
            if (Tools::strlen(Tools::getValue('certissim_authkey')) < 1) {
                $this->_errors[] = $this->l('Authkey can\'t be empty');
            }
        }

        if (Tools::getValue('certissim_franfinance_status')) {
            if (Tools::strlen(Tools::getValue('certissim_franfinance_siteid')) < 1) {
                $this->_errors[] = $this->l('Franfiance siteID can\'t be empty');
            } elseif (!preg_match(
                '#^[0-9]+$#',
                Tools::getValue('certissim_franfinance_siteid')
            )) {
                $this->_errors[] = $this->l('Franfinance Siteid has to be integer.');
            }
        }
        ...
        foreach ($shop_categories as $id => $shop_category) {
            if (!in_array(
                Tools::getValue('certissim_' . $id . '_product_type'),
                array_merge(array_keys($this->product_types), array('0'))
            )) {
        ...
        foreach ($shop_carriers as $id => $shop_carrier) {
            if (!in_array(
                Tools::getValue('certissim_' . $id . '_carrier_type'),
                array_merge(array_keys($this->carrier_types), array('0'))
            )) {
                $this->_errors[] = $this->l('Invalid carrier type for carrier:') . " '" . $shop_carrier['name'] . "'";
            }

            if (!in_array(
                Tools::getValue('certissim_' . $id . '_carrier_speed'),
                array_merge(array_keys($this->carrier_speeds), array('0'))
            )) {
                $this->_errors[] = $this->l('Invalid carrier speed for carrier:')
                    . " '" . $shop_carrier['name'] . "'";
            }
        ...
        foreach ($shop_payments as $id => $shop_payment) {
            if (!in_array(
                Tools::getValue('certissim_' . $id . '_payment_type'),
                array_keys($this->payment_types)
            )) {
    ...
    private function processForm()
    {
        ...
        if ($this->formIsValid()) {
            ...
            Configuration::updateValue(
                'CERTISSIM_LOGIN',
                Tools::getValue('certissim_login')
            );
            Configuration::updateValue(
                'CERTISSIM_PASSWORD',
                Tools::getValue('certissim_password')
            );
            Configuration::updateValue(
                'CERTISSIM_PASSWORDURLENCODED',
                urlencode(Tools::getValue('certissim_password'))
            );
            Configuration::updateValue(
                'CERTISSIM_SITEID',
                Tools::getValue('certissim_siteid')
            );
            Configuration::updateValue(
                'CERTISSIM_AUTHKEY',
                Tools::getValue('certissim_authkey')
            );
            Configuration::updateValue(
                'CERTISSIM_COPILOT_STATUS',
                ((int) Tools::getValue('certissim_copilot_status') == 1 ? '1' : '0')
            );
            Configuration::updateValue(
                'CERTISSIM_FRANFINANCE_SITEID',
                Tools::getValue('certissim_franfinance_siteid')
            );
            Configuration::updateValue(
                'CERTISSIM_FRANFINANCE_STATUS',
                ((int) Tools::getValue('certissim_franfinance_status') == 1 ? '1' : '0')
            );
            Configuration::updateValue(
                'CERTISSIM_STATUS',
                Tools::getValue('certissim_status')
            ...
            /** shop configuration * */
            Configuration::updateValue(
                'CERTISSIM_DEFAULT_PRODUCT_TYPE',
                Tools::getValue('certissim_default_product_type')
            );
            Configuration::updateValue(
                'CERTISSIM_DEFAULT_CARRIER_TYPE',
                Tools::getValue('certissim_default_carrier_type')
            );
            Configuration::updateValue(
                'CERTISSIM_DEFAULT_CARRIER_SPEED',
                Tools::getValue('certissim_default_carrier_speed')
            );
            ...
            foreach (array_keys($shop_carriers) as $id) {
                Configuration::updateValue(
                    'CERTISSIM_' . $id . '_CARRIER_TYPE',
                    Tools::getValue('certissim_' . $id . '_carrier_type')
                );
                Configuration::updateValue(
                    'CERTISSIM_' . $id . '_CARRIER_SPEED',
                    Tools::getValue('certissim_' . $id . '_carrier_speed')
                );
            }
            ...
            foreach (array_keys($payment_modules) as $id) {
                Configuration::updateValue(
                    'CERTISSIM_' . $id . '_PAYMENT_TYPE',
                    Tools::getValue('certissim_' . $id . '_payment_type')
                );
                Configuration::updateValue(
                    'CERTISSIM_' . $id . '_PAYMENT_ENABLED',
                    ((int) Tools::getValue('certissim_' . $id . '_payment_enabled') == 1 ? '1' : '0')
                );
                foreach ($levels as $level) {
                    $levelup = str_replace('/', '_', Tools::strtoupper($level));
                    Configuration::updateValue(
                        $levelup . '_' . $id,
                        ((int) Tools::getValue($level . '_' . $id) == 1 ? '1' : '0')
                    );
                }
            }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSettings')) {
            ...
            } else { //if errors have been encountered while validating the form
        ...
        $certissim_login = Tools::isSubmit('certissim_login') ?
            Tools::getValue('certissim_login') : Configuration::get('CERTISSIM_LOGIN');
        $certissim_password = Tools::isSubmit('certissim_password') ?
            Tools::getValue('certissim_password') : Configuration::get('CERTISSIM_PASSWORD');
        $certissim_siteid = Tools::isSubmit('certissim_siteid') ?
            Tools::getValue('certissim_siteid') : Configuration::get('CERTISSIM_SITEID');
        $certissim_authkey = Tools::isSubmit('certissim_authkey') ?
            Tools::getValue('certissim_authkey') : Configuration::get('CERTISSIM_AUTHKEY');
        $certissim_status = Tools::isSubmit('certissim_status') ?
            Tools::getValue('certissim_status') : Configuration::get('CERTISSIM_STATUS');
        ...
        $certissim_copilot_status = Tools::isSubmit('certissim_copilot_status') ?
            Tools::getValue('certissim_copilot_status') : Configuration::get('CERTISSIM_COPILOT_STATUS');
        $certissim_franfinance_status = Tools::isSubmit('certissim_franfinance_status') ?
            Tools::getValue('certissim_franfinance_status') : Configuration::get('CERTISSIM_FRANFINANCE_STATUS');
        $certissim_franfinance_siteid = Tools::isSubmit('certissim_franfinance_siteid') ?
            Tools::getValue('certissim_franfinance_siteid') : Configuration::get('CERTISSIM_FRANFINANCE_SITEID');

        //admin shop address link and log file url
        if (_PS_VERSION_ < '1.5') {
            $url_log = 'index.php?tab=AdminCertissim&action=viewLog&token='
                . Tools::getAdminTokenLite('AdminCertissim');
            $link_shop_setting = 'index.php?tab=AdminContact&token='
                . Tools::getAdminTokenLite('AdminContact');
        } else {
        ...
        $link_cron_eval = Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__
            . 'modules/' . $this->name . '/getevaluation.php?token=' . $token;
        $link_cron_reeval = Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__
            . 'modules/' . $this->name . '/getreevaluation.php?token=' . $token;
        ...
        $this->smarty->assign(array(
            ...
            'certissim_authkey' => Tools::safeOutput($certissim_authkey),
            'certissim_copilot_status' => Tools::safeOutput($certissim_copilot_status),
            'certissim_franfinance_status' => Tools::safeOutput($certissim_franfinance_status),
            'certissim_franfinance_siteid' => Tools::safeOutput($certissim_franfinance_siteid),
            'certissim_status' => Tools::safeOutput($certissim_status),
            'certissim_level' => $this->certissim_levels,
            ...
            'image_path' => __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/certissim.png',
            'logo_path' => __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/logo.png',
            'logo_account_path' => __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/account.gif',
            ...
            'logo_payments_path' => __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/payments.gif',
            'logo_warning' => __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/no.gif',
    ...
    public function hookNewOrder($params)
    {
        ...
        if (!$select || $count > 0) {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Erreur de verif existence order.
				Select = ' . (string) $select . ' et erreur = '
                . Db::getInstance()->getMsgError() . ' ou commande déjà insérée (count =' . $count . ')'
            );
            return false;
        }
        ...
        if ($found) {
            ...
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                "id_order : " . $params['order']->id . ", Module détecté :" . $payment_name
            );

            //defines the state label according to the status of the payment module (activated for Certissim or not)
            $state_label = (Configuration::get('CERTISSIM_' . $id_module . '_PAYMENT_ENABLED') == '1'
                ? 'ready to send' : 'not concerned');
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'State sélectionné : ' . $state_label
            );
        } else { //if module not found, end of process with log
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Erreur module paiement : module ' . $payment_name . ' 
        ...
        $state_sql = 'SELECT `id_certissim_state` '
    ...
    public function hookPostUpdateOrderStatus($params)
    {
        ...
        if ($order_state->id == 2) {
            ...
            if ($state == 'ready to send') {
                ...
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'Commande ' . $id_order . ' pas dans le bon état pour envoi : ' . $state
                );
            }
    ...
    public function hookAdminOrder($params)
    {
        ...
        if (in_array($payment_type, Fianetfraud::$specific_payments)) {
            return false;
        }
        /*         * ************* MODIF FRANFINANCE *************** */
        ...
        switch ($order_label) {
            ...
            case 'sent':
                ...
                if (_PS_VERSION_ < '1.5') {
                    $url_update = 'index.php?tab=AdminCertissim&action=checkoutScore&id_order='
                        . $params['id_order'] . '&token='
                        . Tools::getAdminTokenLite('AdminCertissim');
                } else {
                    $url_update = $this->context->link->getAdminLink('AdminCertissim')
                        . '&id_order=' . $params['id_order'] . '&action=checkoutScore';
                }
            ...
            case 'scored':
                $template_name = $this->loadScoredTemplate(
                    $params['id_order'],
                    $sac
                );
                break;
            ...
            case 'error':
                $template_name = $this->loadErrorTemplate(
                    $params['id_order'],
                    $sac
                );
                break;
            ...
            case 'not concerned':
                ...
                //if the order has been paid, the template contains a link allowing admin user
                //to send the order to Certissim
                if ($paid) {
                    ...
                    if (_PS_VERSION_ < '1.5') {
                        ...
                        $url_send_order = $this->context->link->getAdminLink('AdminCertissim')
                            . '&action=sendOrder&id_order=' . $params['id_order'];
                    }

                    $this->smarty->assign('url_send_order', $url_send_order);

                    //checks if an error occured while sending the order to Certissim
                    $order_array = $this->orderToArray(
                        $params['id_order'],
                        array('error')
                    );
                    ...
                    if (!is_null($order_array['error']) && $order_array['error'] != '') {
                        $this->smarty->assign(
                            'txt',
                            $this->l('An error has been encountered when the order has been sent to Certissim: ')
                            . '\'' . $order_array['error'] . '\'
                        ...
                        $this->smarty->assign(
                            'txt',
                            $this->l(
                                'The order has been paid with a payment method '
                                . 'that is not configured for fraud screening or an error occured.'
                            )
                        );
                    }
                } else {
                    $this->smarty->assign(
                        'txt',
                        $this->l(
                            'The order has not been paid yet, and the payment method '
                            . 'used is not configured for fraud screening.'
                        )
                    );
                }
                break;
            ...
            default:
                ...
                $this->smarty->assign(
                    ...
                    $this->l(
                        ...
                        . ' and the payment method used is not configured for fraud screening.'
                    )
                );
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    "Statut '$order_label' non reconnu pour la commande " . $params['id_order']
                );
                break;
        }
        ...
        $this->smarty->assign(
            'logo_path',
            __PS_BASE_URI__ . 'modules/' . $this->name . '/views/img/certissim_mini.png'
        );
        ...
        if (version_compare(_PS_VERSION_, '1.5', '<')) {
            return $this->smarty->display(
                ...
                . '/views/templates/hook/' . $template_name . '.tpl'
            );
        }
    ...
    private function loadErrorTemplate($id_order, CertissimSac $sac)
    {
        ...
        if (_PS_VERSION_ < '1.5') {
            $url_update = 'index.php?tab=AdminCertissim&action=checkoutScore&id_order='
                . $id_order . '&token=' . Tools::getAdminTokenLite('AdminCertissim');
        } else {
            $url_update = $this->context->link->getAdminLink('AdminCertissim')
                . "&id_order=$id_order&action=checkoutScore";
        }
    ...
    {
        //loads order params as an array
        $array_order = $this->orderToArray(
            $id_order,
            array('date', 'score', 'profil', 'detail')
        );
        //builds the URL that allow admin user to checkout the order evaluation. Depends on the PS version
        if (_PS_VERSION_ < '1.5') {
            $url_update = 'index.php?tab=AdminCertissim&action=checkoutScore&id_order='
        ...
        $this->smarty->assign(array(
            ...
            'path_to_picto' => _MODULE_DIR_ . 'fianetfraud/views/img/' . $array_order['score'] . '.gif',
    ...
    public function updateOrder($id_order)
    {
        ...
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            'Résultat de lappel pour la commande ' . $id_order
        );
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            $result
    ...
    public static function handleResult($result)
    {
        ...
        if (!$result->hasBeenFound()) {
            ...
            self::updateCertissimOrder(
                $result->returnRefid(),
                array('error' => 'la commande a été envoyée mais n\'est pas retrouvée')
            );
            ...
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Aucune transaction trouvée pour id_order=' . $result->returnRefid()
            );
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Réponse de Certissim : ' . $result
            );
        ...
        switch ($newer_transaction->returnAvancement()) {
            ...
            case 'encours':
                ...
                self::updateCertissimOrder(
                    $result->returnRefid(),
                    array('avancement' => $newer_transaction->returnAvancement())
                );
            ...
            default:
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'Avancement ' . $newer_transaction->returnAvancement() . ' inconnu.'
                );
                break;
        }
    }
    ...
    private function buildXMLOrder($id_order, $from_id_cart = false)
    {
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            'construction du flux pour order ' . $id_order
        );

        if ($from_id_cart == false) {
            $order = new Order($id_order);
            $cart = new Cart($order->id_cart);
        } else {
            $cart = new Cart($id_order);
            $order = $cart;
        }
        ...
        if (_PS_VERSION_ < '1.5') {
            ...
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'id_gender = ' . $customer->id_gender . ', gender name =' . $customer_gender->name[$lang_id]
            );
        ...
        if (_PS_VERSION_ >= '1.5' && Shop::isFeatureActive()) {
            if (Configuration::get('CERTISSIM_FRANFINANCE_STATUS') && in_array(
                $payment_type,
                Fianetfraud::$specific_payments
            )) {
                $siteid = Configuration::get(
                    'CERTISSIM_FRANFINANCE_SITEID',
                    null,
                    null,
                    $order->id_shop
            ...
            if (Configuration::get('CERTISSIM_FRANFINANCE_STATUS') && in_array(
                ...
                $siteid = Configuration::get('CERTISSIM_FRANFINANCE_SITEID');
            } else {
        ...
        if ($from_id_cart) {
            $ref = $cart->id;
            $total_paid = (string) $cart->getOrderTotal();
            ...
            $ip_customer = self::getIpByOrder((int) $order->id);
        ...
        if (_PS_VERSION_ >= '1.5' && Shop::isFeatureActive()) {
            $default_product_type = Configuration::get(
                'CERTISSIM_DEFAULT_PRODUCT_TYPE',
                null,
                null,
                $order->id_shop
        ...
        foreach ($products as $product) {
            ...
            if ($from_id_cart) {
                ...
                $product_price = $product['total_wt'];
                $product_name = $product['name'];
                $alldownloadables = false;
            } else {
            ...
            if (_PS_VERSION_ >= '1.5' && Shop::isFeatureActive()) {
                $product_type = Configuration::get(
                    'CERTISSIM' . $product_category . '_PRODUCT_TYPE',
                    null,
                    null,
                    $order->id_shop
                );
            ...
            if ($product_type) {
                $xml_element_product->addAttribute(
                    'type',
                    Configuration::get(
                        'CERTISSIM' . $product_category . '_PRODUCT_TYPE',
                        null,
            ...
            $xml_element_product->addAttribute(
                'ref',
                CertissimTools::normalizeString($product_ref)
            );
            $xml_element_product->addAttribute(
                'nb',
                $product_quantity
        ...
        if ($alldownloadables) {
            ...
            if (in_array(
                Configuration::get(
                    'CERTISSIM_' . (string) $carrier->id . '_CARRIER_TYPE',
                    null,
                    null,
                    $order->id_shop
                ),
                ...
                $real_carrier_type = Configuration::get(
                    'CERTISSIM_' . (string) $carrier->id . '_CARRIER_TYPE',
                    null,
                    null,
                    $order->id_shop
                );
                $real_carrier_speed = Configuration::get(
                    'CERTISSIM_' . (string) $carrier->id . '_CARRIER_SPEED',
                    null,
                    null,
                    $order->id_shop
                );
            ...
            Configuration::get('CERTISSIM_' . (string) $carrier->id . '_CARRIER_TYPE'),
            array_keys($this->carrier_types)
        )) {
        ...
        switch ($real_carrier_type) {
            ...
            case '6':
            case '10':
                $socolissimoinfo = $this->getSoColissimoInfo(
                    $order->id_cart,
                ...
                if ($socolissimoinfo != false) {
                    foreach ($socolissimoinfo as $info) {
                        ...
                        if (version_compare(
                            $socolissimo_installed_module->version,
                            '2.8',
                            '<'
                        )) {
                            $address2 = $address1;
                    ...
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'Utilisation transporteur So Colissimo, livraison : ' . $delivery_mode
                    );

                    //if delivery mode is DOM or RDV, <adresse type="livraison" ...>
                    //and <utilisateur type="livraison" ...> added
                    if ($delivery_mode == 'DOM' || $delivery_mode == 'RDV') {
                        ...
                        $xml_pointrelais = new CertissimPointrelais(
                            null,
                            $address1,
                            $adressepointrelais
                        );
                    ...
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'Flux incorrect : Module SoColissimo non installé ou non activé'
                    );
                }
                break;
            ...
            case '8':
                ...
                if ($mondialrelayinfo != false) {
                    ...
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'Flux incorrect : Module Mondial relay non installé ou non activé'
                    );
                }
                break;
            case '9':
                ...
                if (Module::isInstalled('chronopost') || $chronopost_is_enabled) {
                    ...
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'Flux incorrect : Module Chronopost non installé ou non activé'
                    );
                }
                break;
            ...
            case '11':
                ...
                if ($this->checkModuleisEnabledOrInstalled('icirelais')) {
                    ...
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'Flux incorrect : Module Ici Relais non installé ou non activé'
                    );
                }
                break;
            ...
            default:
                ...
                if ($real_carrier_type == 1) {
                    ...
                    $delivery_address = $this->getDefautAddress(
        ...
        if ($from_id_cart == false) {
            ...
            if (_PS_VERSION_ >= '1.5' && Shop::isFeatureActive()) {
                $payment_type = $this->payment_types[Configuration::get(
                    'CERTISSIM_' . $id_payment_module . '_PAYMENT_TYPE',
                    null,
                    null,
                    $order->id_shop
                )];
            ...
            if ($auth != '') {
                $xml_element_payment->addAttribute('auth', $auth);
            }

            /*             * ************* MODIF FRANFINANCE *************** */
            if (Configuration::get('CERTISSIM_FRANFINANCE_STATUS')) {
                if ($order->module == 'franfinance') {
                    //3xWEB, 4xWEB or SRINT SECURE
                    $xml_element_payment->childNom($this->getFranfinanceSolution($id_order));
                }
            }
            /*             * ************* MODIF FRANFINANCE *************** */
        ...
        if ($from_id_cart && Configuration::get('CERTISSIM_COPILOT_STATUS')) {
            $stack = new CertissimPaymentAuthRequest($ref);
        ...
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            '---- flux généré pour commande ' . $id_order . ' ----'
        );
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            $xml_element_control->getXML()
        );
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            '---------------------------------------'
    ...
    private function sendXMLOrder($stack, $id_order)
    {
        ...
        if (Configuration::get('CERTISSIM_FRANFINANCE_STATUS') && in_array(
            ...
            $res = $sac->sendStacking(
                $stack,
                Configuration::get('CERTISSIM_FRANFINANCE_SITEID')
            );
        } else {
        ...
        if ($res === false) { //connection to Certissim failed
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Connexion échouée pour la commande ' . (int) $order->id
            );
            return false;
        }
        if ($validstack->hasFatalError()) { //connexion to Certissim failed
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Envoi échoué pour la commande ' . (int) $order->id
            );
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Retour du script :'
            ...
            self::updateCertissimOrder(
                $id_order,
                array('avancement' => 'error', 'error' => $validstack->getError())
            );
        ...
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            '---- Retour de Fia-Net pour la commande ' . (int) $order->id . ' ----'
        );
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            $res
        ...
        foreach ($validstack->getResults() as $result) {
            ...
            switch ($result->returnAvancement()) {
                ...
                case 'encours':
                    ...
                    self::updateCertissimOrder(
                        $id_order,
                        array('avancement' => $result->returnAvancement())
                    );
                ...
                case 'error':
                    ...
                    self::updateCertissimOrder(
                        $id_order,
                        array('avancement' => $result->returnAvancement(), 'error' => $result->getDetail())
                    );
                ...
                default:
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'Avancement ' . $result->returnAvancement() . ' inconnu.'
                    );
                    break;
            }
        }
    ...
    public static function switchOrderToState($id_order, $state_label)
    {
        ...
        self::updateCertissimOrder(
            $id_order,
            array('id_certissim_state' => $id_state)
        );
    }
    ...
    /**
     * inserts a certissim order into the certissim_order table with the fields 
     * given as a parameter and return true if success, false otherwise
     * 
    ...
    private static function insertCertissimOrder(array $fields)
    {
        ...
        $fields = array_map('pSQL', $fields);
        //generates the part of the SQL string that defines the field names to insert
        $fieldnames = implode('`,`', array_keys($fields));
        //generates the part of the SQL string that defines the field values
        $fieldvalues = implode("','", $fields);
        $sql = 'INSERT INTO `' . _DB_PREFIX_ . self::CERTISSIM_ORDER_TABLE_NAME . "` (`$fieldnames`) "
            . "VALUES ('$fieldvalues')"; //builds the total SQL string
        $inserted = Db::getInstance()->execute($sql); //execute the SQL query
        //log
        if (!$inserted) {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Insertion échouée pour la requête : ' . $sql
            );
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Insertion OK pour la requête : ' . $sql
            );
        }
    ...
    public static function updateCertissimOrder($id_order, array $fields, $update_by_id_cart = false)
    {
        ...
        foreach ($fields as $fieldname => $fieldvalue) {
            $set_string .= "`$fieldname`='" . pSQL(
                CertissimTools::convertEncoding(
                    $fieldvalue,
        ...
        if (!$update_by_id_cart) {
            $sql = 'UPDATE `' . _DB_PREFIX_ . self::CERTISSIM_ORDER_TABLE_NAME . "` "
                . "SET $set_string "
        ...
        if (!$updated) {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Mise à jour échouée pour la requête : ' . $sql
            );
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Mise à jour OK pour la requête : ' . $sql
            );
        }
    ...
    public function getIciRelaisInfo($id_cart)
    {
        if ($this->checkModuleisEnabledOrInstalled('icirelais')) {
            ...
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Module Ici Relais non installé ou non activé'
            );
            return false;
        }
    }
     ...
     * 
     * @param mixed $id_shop, id_shop of the shop concerned, or null 
     * if no shop concerned particularly (PS 1.4 or multishop not available)
     */
    public static function getReevaluations($id_shop = null)
    {
        ...
        CertissimLogger::insertLog(
            __METHOD__ . ' : ' . __LINE__,
            $reevals->returnCount() . ' réévaluations trouvées.'
        ); //log
        ...
        foreach ($reevals->getTransactions() as $transaction) {
            ...
            if (!$update) {
                CertissimLogger::insertLog(
                    ...
                    . '' . $transaction->returnRefid() . ' : ' . Db::getInstance()->getMsgError()
                );
            } else {
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'Commande ' . $transaction->returnRefid() . ' réévaluée.'
                );
            }
        }
    }
    ...
    public static function getEvaluations()
    {
        ...
        if (_PS_VERSION_ < '1.5' || !Shop::isFeatureActive()) {
            ...
            foreach ($lists as $list) {
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'Ref list : ' . implode(', ', $list)
                );
                //calls Certissim webservice
                $response = $sac->getValidstackByReflist(
                    $list,
                    CertissimSac::CONSULT_MODE_MINI
                );
            ...
            foreach (Shop::getShops() as $shop) {
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'Récupération des évaluations en attente pour shop ' . $shop['id_shop']
                ); //finlog
                $sql_orders = '
							SELECT o.`id_order` as `id_order`
							FROM `' . _DB_PREFIX_ . Fianetfraud::CERTISSIM_ORDER_TABLE_NAME . '` o 
    ...
    private function checkCartID($id_cart)
    {
        ...
        if ($query_result != null) {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'id_cart found :' . $id_cart
            );
            return true;
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'id_cart not found, id_cart added :' . $id_cart
            );
            return false;
        }
    }
    ...
    public function saveCertissimOrders()
    {
        ...
        if ($exist) {
            ...
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'certissim_orders_temp table created'
            );
            return true;
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'certissim_orders_temp table not created'
            );
            return false;
        }
    }
    ...
    public function hookBackBeforePayment($params)
    {
        ...
        if (Configuration::get('CERTISSIM_COPILOT_STATUS')) {
            ...
            $xml_ligth = $this->removeXMLTimestamp($xml->getXML());

            if (isset($this->context->cookie->session_xml)) {
                $xml_session_ligth = $this->removeXMLTimestamp($this->context->cookie->session_xml);
            } else {
                $xml_session_ligth = null;
            }


            if (!isset($this->context->cookie->session_xml)
                || ($xml_ligth != $xml_session_ligth)
                || !$this->getAuthID($id_cart)) {
                $result_copilot = $this->sendCopilotXMLOrder($xml, $id_cart);

                if ($result_copilot) {
                    $authlevel = $result_copilot['authlevel'];
                    $authid = $result_copilot['authid'];
                } else {
                    $authlevel = 'high/required';
                    $authid = '';
                }
            } else {
                $authlevel = $this->context->cookie->session_authlevel;
                $authid = $this->context->cookie->session_authid;
            }

            //affichage des paiements en fonction de la reponse copilot
            $shop_payments = $this->loadPaymentMethods();

            foreach (array_keys($shop_payments) as $id) {
                $label_conf = Tools::strtoupper(str_replace('/', '_', $authlevel) . '_' . $id);

                if (!Configuration::get($label_conf)) {
                    $mod = Module::getInstanceById($id);
                    $mod->active = false;
                }
            }

            $this->context->cookie->__set('session_authlevel', $authlevel);
            $this->context->cookie->__set('session_authid', $authid);
            $this->context->cookie->__set('session_xml', $xml->getXML());

            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'id_cart : ' . $id_cart . ', authID : ' . $authid
            );
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'id_cart : ' . $id_cart . ', authLevel : ' . $authlevel
            );
    ...
    private function sendCopilotXMLOrder($stack, $id_cart)
    {

        //initializes Certissim
        if (_PS_VERSION_ >= '1.5' && Shop::isFeatureActive()) {
            $sac = new CertissimSac(Context::getContext()->shop->id);
        } else {
            $sac = new CertissimSac();
        }

        //sends the stream and catches the response
        $validcopilot = $sac->sendCopilot($stack);

        if ($validcopilot != false) {
            if (CertissimTools::getHttpResponse($validcopilot['header']) == '200') {
                if (CertissimTools::isXMLstring($validcopilot['response'])) {
                    $response = new CertissimPaymentAuthResponse($validcopilot['response']);
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'id_cart ' . $id_cart . ' : reponse http : '
                        . $validcopilot['header'] . ', reponse copilot : ' . $validcopilot['response']
                    );
                    return array('authlevel' => $response->getAuthLevel(), 'authid' => $response->getAuthId());
                } else {
                    CertissimLogger::insertLog(
                        __METHOD__ . ' : ' . __LINE__,
                        'id_cart ' . $id_cart . ' : chaine retour pas au format xml'
                    );
                    return false;
                }
            } else {
                CertissimLogger::insertLog(
                    __METHOD__ . ' : ' . __LINE__,
                    'id_cart ' . $id_cart . ' :'
                    . ' erreur http ' . CertissimTools::getHttpResponse($validcopilot['header']) . ','
                    . ' libellé erreur : ' . $validcopilot['response']
                );
                return false;
            }
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'id_cart ' . $id_cart . ' : erreur de connexion fsockopen'
            );
            return false;
        }
    }

    private function getAuthLevel($id_cart)
    {
        $sql = 'SELECT `authlevel` '
            . 'FROM `' . _DB_PREFIX_ . self::CERTISSIM_ORDER_TABLE_NAME . "` "
            . "WHERE `id_cart` = '" . (int) $id_cart . "'";
        $query_result = Db::getInstance()->getRow($sql);
        if ($query_result['authlevel'] != null || $query_result['authlevel'] != '') {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'authlevel found for id_cart :' . $id_cart
            );
            return $query_result['authlevel'];
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'authlevel not found for id_cart :' . $id_cart
            );
            return false;
        }
    }

    private function getAuthID($id_cart)
    {
        $sql = 'SELECT `authid` '
            . 'FROM `' . _DB_PREFIX_ . self::CERTISSIM_ORDER_TABLE_NAME . "` "
            . "WHERE `id_cart` = '" . (int) $id_cart . "'";
        ...
        if ($query_result['authid'] != null || $query_result['authid'] != '') {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'authid found for id_cart :' . $id_cart
            );
            return $query_result['authid'];
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'authid not found for id_cart :' . $id_cart
            );
            return false;
        }
    }

    private function removeXMLTimestamp($string)
    {
        $findme = 'timestamp';
        $start = strpos($string, $findme);
        $xml_size = Tools::strlen($string);
        $diff = (Tools::strlen($string) - $start) - 32;
        $end = $xml_size - ($start + $diff);

        $start--;

        $string_light = substr_replace($string, '', $start, $end);

        return $string_light;
    }

    private function getFranfinanceSolution($id_order)
    {
        $sql = 'SELECT `solution` '
            . 'FROM `' . _DB_PREFIX_ . self::FRANFINANCE_ORDER . "` "
            . "WHERE `id_order` = '" . (int) $id_order . "'";
        ...
        if ($query_result) {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Solution Franfinance trouvée pour id_order = ' . $id_order . ', solution : ' . $query_result
            );

            if ($query_result == '3XWeb') {
                return '3xWEB';
            }
            if ($query_result == '4XWeb') {
                return '4xWEB';
            }
            if ($query_result == 'SprintSecure') {
                return 'SPRINT SECURE';
            }
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Solution Franfinance non trouvée pour id_order = ' . $id_order
            );
            return false;
        }
    }

    private function getFranfinanceOrdersToSend()
    {
        ...
        if ($query_result) {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Transactions Franfinance à envoyer'
            );
            return $query_result;
        } else {
            CertissimLogger::insertLog(
                __METHOD__ . ' : ' . __LINE__,
                'Aucune transaction Franfinance à envoyer'
            );
            return false;
        }
    }
